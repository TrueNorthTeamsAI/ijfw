...
Wayland invokes IJFW via the `ijfw` CLI (e.g. `ijfw status`, `ijfw doctor`). Intent phrases trigger the same flows ("recall", "cross-audit", "handoff").
...
IJFW ships 19 skills under `~/.wayland/skills/ijfw-*`: status, handoff, cross-audit, compress, debug, design, workflow, commit, doctor, update, summarize, dashboard, preflight, critique, recall, review, team, memory-audit, plan-check. Skills hot-load on trigger and unload when done -- zero resident context cost.
