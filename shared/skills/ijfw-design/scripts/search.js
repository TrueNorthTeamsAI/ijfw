...
//   node search.js "<query>" --mockup -o <output-dir> [--layout tabs|sidebar|cards]
//   node search.js --gallery -o <output-dir>
//
...
import { generateMockup, generateGallery } from './mockup-generator.js';
...
if (argv.length === 0) {
  ...
  console.log('       node search.js "<query>" --mockup -o <output-dir> [--layout tabs|sidebar|cards]');
  console.log('       node search.js --gallery -o <output-dir>');
  process.exit(0);
}
...
for (let i = 0; i < argv.length; i++) {
  ...
  else if (a === '--mockup') mockup = true;
  else if (a === '--gallery') gallery = true;
  ...
  else if ((a === '-o' || a === '--output') && argv[i + 1]) { outputDir = argv[++i]; }
  else if (a === '--layout' && argv[i + 1]) { layout = argv[++i]; }
}

let output;

if (gallery) {
  const dir = outputDir || './gallery';
  const styles  = loadCSV('styles.csv');
  const palettes = loadCSV('palettes.csv');
  const outPath = generateGallery(styles, palettes, dir);
  console.log('Gallery generated: ' + outPath + ' (' + styles.length + ' styles)');
  process.exit(0);
}

if (mockup) {
  const dir = outputDir || './mockup';
  const styles   = loadCSV('styles.csv');
  const palettes = loadCSV('palettes.csv');
  const typo     = loadCSV('typography.csv');
  const reasoning = loadCSV('reasoning.csv');

  const query = queryArg || 'dashboard';
  const ruleMatch = search(reasoning, query, 1);
  const rule = ruleMatch[0] || null;
  ...
  const outPath = generateMockup(query, matchedPalette, matchedTypo, matchedStyle, layout, dir);
  const layoutLabel = layout ? ` (${layout} layout)` : ' (design system reference)';
  console.log('Mockup generated' + layoutLabel + ': ' + outPath);
  process.exit(0);
}
